...
export function validateWorkflowResponse(p){
 ...
 const dirs=Array.isArray(p.solution_directions)?p.solution_directions:[];if(p.response_type==='direction_selection'){if(dirs.length<1||dirs.length>4)add('solution_directions','必须包含 1—4 个方向');if(p.recommendation_status!=='waiting_for_selection')add('recommendation_status','必须为 waiting_for_selection');if(txt(p.selected_direction))add('selected_direction','用户选择前不得预填');dirs.forEach((v,i)=>{for(const field of ['id','name','description','target_problem','mechanism','expected_improvement','source_rule_id'])if(!txt(v?.[field]))add(`solution_directions[${i}].${field}`,'方向展示字段必须完整且可追溯');});}
 ...
 if(p.response_type==='product'){if(!products.length||products.length>3)add('recommended_products','商品响应必须包含 1—3 个商品');products.forEach((v,i)=>{if(!txt(v?.source_id))add(`recommended_products[${i}].source_id`,'商品必须可追溯');for(const field of ['official_website','purchase_url'])if(v?.[field]!==null&&v?.[field]!==undefined&&!/^https?:\/\//i.test(v[field]))add(`recommended_products[${i}].${field}`,'渠道地址必须是真实 HTTP(S) URL 或 null');});}
