...
- After every completed change, create a corresponding Git commit so the change can be traced and rolled back. Keep each commit scoped to the current task and use a descriptive commit message.
- After every change, add or update the relevant tests. Before handing work to the user, run all applicable tests, the mobile runtime integrity check, and the production build; all validations must pass.
- The same requirements are recorded in `AGENT.md` for human-readable project documentation.

## Prototype Instructions

### Product design decisions

- Phase 1 ships as a responsive web application hosted online; the phone frame is a local preview surface only. A WeChat Mini Program adaptation is a separate later phase and should not constrain the current web implementation.

- Keep the homepage conversation-first and visually similar in simplicity to familiar AI chat products.
- Use a warm pig-skin-inspired palette: blush, peach, cream, muted coral, warm taupe, and warm charcoal. Do not mix in green accents.
- Use the exact flat veterinarian pig avatar from `public/assistant-pig-flat-source.png`; do not substitute a newly generated 3D or big-eye pig avatar.
- Use natural farmer-facing language; avoid the formal term “猪只” in user-facing copy.
- Current slogan: “让每一次养殖判断，都更有依据”.
- Keep all app-owned top navigation below the device status bar / Dynamic Island safe region.
- Do not show the “问症状 / 拍照分析 / 查产品” shortcut row on the homepage; the composer remains the single persistent action.
...
- Mobile is the primary density baseline: keep the production phone homepage compact (58px app bar, 72px avatar, 21px welcome heading, 54px prompt rows) and let desktop expand the workspace without scaling mobile typography and spacing wholesale.
- Mobile density rules apply to the entire workflow, not only the homepage: question, answered-state, diagnosis, direction, management, product, terminal, and error cards must all have phone-specific typography and spacing verified through a real multi-step browser test.
